...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/02/26 18:57:21 by mpaziuk           #+#    #+#             */
/*   Updated: 2017/02/26 18:57:22 by mpaziuk          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

inline static void	calc(t_param *p, t_complex *z, t_complex *c)
{
	register int	i;
	double			r_init;
...
int					draw_mandel(t_env *e)
{
	...
	while (++i < THREAD)
	{
		...
		pthread_create(threads + i, NULL, ft_draw, &(params[i]));
	}
	i = -1;
	while (++i < THREAD)
